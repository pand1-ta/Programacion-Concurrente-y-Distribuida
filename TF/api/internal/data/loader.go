...
func LoadMapping(path string) (map[string]int, map[int]string, error) {
	...
	for {
		...
		if len(row) < 2 {
			return nil, nil, fmt.Errorf("invalid mapping row %d: %v", lineNum, row)
		}
		...
		if _, errA := strconv.Atoi(row[0]); errA == nil {
			...
			if _, errB := strconv.Atoi(row[1]); errB != nil {
				return nil, nil, fmt.Errorf("invalid mapping at row %d: neither column is numeric: %v", lineNum, row)
			}
